...
export default function ReceivedCVsPage() {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'shortlisted': { label: 'Shortlisted', class: 'bg-accent-100 text-accent-800 border-accent-200' },
      'rejected': { label: 'Rejected', class: 'bg-red-100 text-red-800 border-red-200' },
      'interview': { label: 'Interview', class: 'bg-warm-100 text-warm-800 border-warm-200' },
  ...
  return (
    <div className="max-w-7xl mx-auto">
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Received CVs</h1>
          <p className="text-gray-600">View and manage CVs received from candidates</p>
        </div>
                    ...
                    <tr key={cv.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          ...
                          <button
                            onClick={() => handleAction(cv, 'interview')}
