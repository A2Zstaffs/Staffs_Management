...
export default function RecruiterManagerDashboard() {
        ...
        {stats.map((stat, index) => {
            ...
            return (
                <Link href={stat.link} key={index}>
                    <div className="bg-gradient-to-br from-blue-50 to-white border-2 border-gray-200 rounded-2xl p-6 hover:shadow-lg hover:border-blue-300 transition-all cursor-pointer">
    ...
    <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6">
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-900">Your Assigned Recruiters</h2>
            <Link
        ...
        <div className="space-y-4">
            {dashboardData?.recruiters && dashboardData.recruiters.length > 0 ? (
                dashboardData.recruiters.map((recruiter) => (
                    <div key={recruiter._id} className="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:bg-gray-100 transition-colors">
                        <div className="flex justify-between items-center">
                            <div>
                                <h3 className="font-semibold text-gray-900">{recruiter.fullName}</h3>
    ...
    <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <Link href="/recruiter-manager/jobs">
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6 hover:shadow-lg hover:border-purple-300 transition-all cursor-pointer">
                <Briefcase className="w-8 h-8 text-purple-600 mb-3" />
                <h3 className="text-lg font-semibold text-gray-900 mb-2">View All Jobs</h3>
                <p className="text-sm text-gray-500">Access all active job postings</p>
            </div>
        </Link>

        <Link href="/recruiter-manager/candidates">
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6 hover:shadow-lg hover:border-green-300 transition-all cursor-pointer">
                <FileText className="w-8 h-8 text-green-600 mb-3" />
                <h3 className="text-lg font-semibold text-gray-900 mb-2">View Profiles</h3>
                <p className="text-sm text-gray-500">Review profiles uploaded by your recruiters</p>
            </div>
        </Link>

        <Link href="/recruiter-manager/applications">
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6 hover:shadow-lg hover:border-orange-300 transition-all cursor-pointer">
                <TrendingUp className="w-8 h-8 text-orange-600 mb-3" />
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Track Applications</h3>
                <p className="text-sm text-gray-500">Monitor application status and performance</p>
            </div>
        </Link>
    </div>
                    ...
                    <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6">
                        ...
                        <div className="space-y-3">
                            {dashboardData?.recruiters
                                ...
                                .slice(0, 3)
                                .map((recruiter, index) => (
                                    <div key={recruiter._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                        ...
                                        <div className="text-right">
                                            <p className="font-bold text-blue-600">{recruiter.profileCount || 0}</p>
