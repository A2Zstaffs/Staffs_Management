...
export default function RecruiterSignupPage() {
  ...
  const handleGoogleSignup = async (credentialResponse) => {
    ...
    try {
      const response = await authAPI.googleAuth(credentialResponse.credential, 'recruiter');

      if (response.success) {
        // Use actual role from backend response (handles existing accounts with different roles)
        const actualRole = response.data?.role || response.user?.role || 'recruiter';
        ...
        setTimeout(() => {
          router.push(dashboardRoutes[actualRole] || '/recruiter/home');
        }, actualRole !== 'recruiter' ? 1500 : 100);
      } else {
    ...
    <div className="h-screen overflow-hidden flex items-center justify-center p-4 bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 -mt-16 pt-16">
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white/20 backdrop-blur-xl rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] border border-white/40 p-6">
          ...
          {/* Signup Heading */}
          <h1 className="text-2xl font-bold text-secondary-800 mb-6 text-center">Join as Recruiter</h1>
